...
| [paseo](https://github.com/getpaseo/paseo) | Paseo desktop app (Electron). | GitHub AppImage → `~/Applications` + app menu entry | — |
...
- AppImages (`appimage:` method) land in `~/Applications` under the release
  asset's name, so re-running replaces the old version in place. The installer
  makes sure FUSE 2 (`libfuse2t64`) is present, which AppImages need to run.
  It then writes `~/.local/share/applications/<name>.desktop` from the
  AppImage's own `.desktop` file and icon, keeping its launch flags (Paseo's
  includes Electron's `--no-sandbox`), so the app shows up in the rofi launcher
  (**≡** on the bar).
- Adding an app: add a `name="method:spec"` entry to the `APPS` map in
  ...
  `flatpak:app.id`, `appimage:owner/repo:asset`, `custom:function`.
